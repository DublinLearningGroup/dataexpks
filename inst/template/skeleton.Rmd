...
names(rawdata_tbl) <- rawdata_tbl %>% names %>% clean_names
...
missing_univariate_tbl <- rawdata_tbl %>%
    summarise_each(funs(sum(is.na(.)))) %>%
    gather('variable','missing_count')

ggplot(missing_univariate_tbl) +
    geom_bar(aes(x = fct_reorder(variable, -missing_count), weight = missing_count)) +
...
if(ncol(data_tbl) > dataexp_pairsplot_colcount ||
   nrow(data_tbl) > dataexp_pairsplot_rowcount) {
    ...
    if(any(sapply(data_tbl, is.logical))) {
        conv_tbl <- data_tbl %>%
...
rm(plot_var, outlier_point, plot_tbl)
```
...
if(nrow(data_tbl) > mds_sample_count) {
    for(i in 1:mds_iter_count) {
        cat("--\n")
        cat(paste0("MDS plot iter: ", i, "\n"))

        sample_tbl <- data_tbl %>%
            filter(row_ids) %>%
            sample_n(dataexp_multivar_rowcount)
        
        num_mds_dist <- sample_tbl %>% dist
...
if(nrow(data_tbl) > tsne_sample_count) {
    for(i in 1:tsne_iter_count) {
        cat("--\n")
        cat(paste0("t-SNE plot iter: ", i, "\n"))

        tsne_tbl <- sample_tbl %>%
            sample_n(tsne_sample_count)
...
#num_data_tbl <- data_tbl %>% select(one_of(numeric_vars)) %>% filter(complete.cases(.))


num_data_tbl <- data_tbl %>%
    select(prem_ape, policy_duration, mort_rating, sum_assured)
