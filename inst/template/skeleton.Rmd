...
numeric_vars <- numeric_vars[!numeric_vars %in% facet_varname]
...
datetime_vars <- datetime_vars[!datetime_vars %in% facet_varname]
...
```{r outlier_univariate_numeric_plots, echo=TRUE, warning=FALSE}
for(plot_varname in numeric_vars) {
    cat("--\n")
    cat(paste0(plot_varname, '\n'))
...
```{r estimate_data_mean_covariance, echo=TRUE, cache=TRUE}
...
mcd_estimate <- num_data_tbl %>%
    ...
    MASS::cov.rob(cor = TRUE, method = 'mcd')
```
