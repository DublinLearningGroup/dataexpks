...
#rawdata_tbl <- read_csv(DATA_FILE, progress = FALSE)
...
if((data_tbl %>% filter(row_ids) %>% nrow) > mds_sample_count) {
...
if((data_tbl %>% filter(row_ids) %>% nrow) > tsne_sample_count) {
    sample_tbl <- data_tbl %>%
