...
create_ggpairs_tbl <- function(data_tbl, sample_cols, sample_rows) {
    col_index <- sample(ncol(data_tbl), sample_cols, replace = FALSE) %>%
        sort

    sample_tbl <- data_tbl %>%
        mutate_if(function(x) is.character(x) || is.factor(x)
...
### Data is loaded into dataset data_tbl here
#rawdata_tbl
...
coltype_lst <- create_coltype_list(rawdata_tbl)

catvar_valuecount_tbl <- rawdata_tbl %>%
...
Having removed the unique identifier variables from the dataset, we now can continue doing some basic exploration of the data. We may also choose to remove some extra columns from the dataset.

```{r select_exploration_variables, echo=TRUE, results='hide'}
...
for(plot_varname in numeric_vars) {
    ...
    explore_plot <- ggplot() +
        ...
        scale_y_continuous(labels = scales::comma) +
        ggtitle(paste0('Histogram Plot for Variable: ', plot_varname, " (", na_count, " missing values)")
...
The technique is useful though, so to circumvent these issues we sample the dataset. We select some random columns and rows, and make a pairsplot of this subset, then repeat this process for a number of samples.

```{r plot_pairsplot, echo=TRUE, warning=FALSE, message=FALSE, fig.width = 20, fig.height=15, cache=TRUE}
